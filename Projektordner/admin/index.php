...
if (!isset($_SESSION["user"]) || $_SESSION["role"] != "admin"){ //Ist die Session nicht gesetzt oder der Nutzer kein Admin wird man auf die Login Seite weitergeleitet.
    ...
    function viewUSER($conn){   //Gibt alle Nutzer in einer Tabelle aus.
        $query = $conn->query("SELECT ID,firstname,lastname,username,role FROM user");
        while($row = $query->fetch_array()){
            if($row["role"] == "admin"){continue;} //Admin Nutzer werden nicht aufgeführt.
            echo "<tr>".
            "<td>".$row['ID']."</td>".
             "<td>".$row['username']."</td>".
             "<td>".$row['firstname']."</td>".
...
<html lang="de">
    <head>
        ...
        <nav class="navbar navbar-default">
            ...
            <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-home"></span> Home | Benutzerverwaltung</button></form>
             </div>  </div>
