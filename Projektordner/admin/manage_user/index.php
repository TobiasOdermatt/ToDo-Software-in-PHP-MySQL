<?php
  ...
  if (!isset($_SESSION["user"]) || $_SESSION["role"] != "admin"){ //Ist die Session nicht gesetzt oder der Nutzer kein Admin wird man auf die Login Seite weitergeleitet.
    ...
    function delUSER($user_id,$conn){ //Löscht einen User wenn die ID mitgegeben wurd
      ...
      }else{header("location: ../index.php?status=delfail");}//War die Löschung nicht Erfolgreich wird dies der Startseite mitgeteilt
    }
    ...
    function existID($user_id,$conn){ //Überprüft ob eine User ID in der Datenbank existiert returnt true oder false
      global $error;
  ...
  if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(isset($_POST['writemode'])){//Dient zur unterscheidung von Nutzer erstellen und Bearbeiten Modus
      if($_POST['writemode'] == "submit"){  //Beim Erstellen
          ...
          $stmt->close();
          $conn->close();
          $alert_class_name = "alert alert-success";$alert_message = "Der Nutzer wurde erfolgreich angelegt.";}else
          ...
          $stmt->close();
          $conn->close();
          $alert_class_name = "alert alert-success";$alert_message = "Der Nutzer wurde erfolgreich bearbeitet.";}}
      }}}
    ...
    elseif(isset($_GET['DEL'])){  //Sollte ein Beitrag gelöscht werden, dann ist die Variabel DEL gesetzt.
        delUSER($_GET['DEL'],$conn);}}  //Funktion für die Löschung wird gestartet, die Variabel DEL wird mitgegeben.
...
<html lang="de">
    <head>
        ...
        <nav class="navbar navbar-default">
            ...
            <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-tag"> </span> Kategorie erstellen</button></form>
                   <!-- Manage User -->
            <form style="display: inline" action="../manage_user/" method="get">
            <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-user"></span> Benutzer erstellen</button></form>
