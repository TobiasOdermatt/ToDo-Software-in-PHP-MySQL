...
function categoryExists($category,$conn){ //Überprüft ob eine Kategorie unter dem Namen schon existiert
...
function existID($ID,$conn){ //Überprüft ob die ID einer Kategorie  existiert
...
function delCategory($ID,$conn){ //Löscht eine Kategorie/sowie dazu gehörigen ToDos, die ID muss mitgegeben werden
    if(existID($ID,$conn)){ //Überprüft ob die ID existiert
      ...
      }else{"location: ../view_category?status=delfail";} //Existiert die ID nicht wird dies der view_category Seite mitgeteilt
}
function editCategory($ID,$conn){ //Lädt die zu bearbeitende Kategorie, ID wird mitgegeben
    if(existID($ID,$conn)){ //Überprüft ob die ID existiert.
            ...
            if(isset($_POST['writemode'])){//Dient zur unterscheidung von Kategorie erstellen und Bearbeiten Modus
                if($_POST['writemode'] == "submit"){//Beim erstellen
                    if(!(categoryExists($category,$conn))){ //Überprüft ob der Name existiert, falls ja wird dies ausgegeben.
                        ...
                        $stmt->close();
                        $conn->close();
                        $alert_class_name = "alert alert-success";$alert_message = "Die Kategorie wurde erfolgreich angelegt.";}
                    ...
                    $stmt->close();
                    $conn->close();
                    $alert_class_name = "alert alert-success";$alert_message = "Die Kategorie wurde erfolgreich bearbeitet.";}}
            }
          }else{$alert_message = $error;}}
...
elseif(isset($_GET['DEL'])){  //Sollte ein Beitrag gelöscht werden, dann ist die Variabel DEL gesetzt.
    ...
    delCategory($delID,$conn);}}  //Funktion für die Löschung wird gestartet, die Variabel DEL wird mitgegeben.
...
<html lang="de">
    <head>
        ...
        <nav class="navbar navbar-default">
            ...
            <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-home"></span> Home | Benutzerverwaltung</button></form>
             </div>  </div>
            ...
            <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-inbox"></span> Kategorien ansehen</button></form>

            <form style="display: inline" action="../manage_category/" method="get">
            <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-tag"> </span> Kategorie erstellen</button></form>

            <form style="display: inline" action="../manage_user" method="get">
            <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-user"></span> Benutzer erstellen</button></form>
    ...
    <div class="form-group"><div class="<?php echo $alert_class_name;?>" role="alert"><?php echo $alert_message; ?></div></div>
        ...
        <div class="form-group">
          ...
          <input type="text" name="category" class="form-control" id="category"
                  ...
                  placeholder="Geben Sie eine Kategorie ein, maximal 25 Zeichen."
                  maxlength="25"
                  required>
        </div>
