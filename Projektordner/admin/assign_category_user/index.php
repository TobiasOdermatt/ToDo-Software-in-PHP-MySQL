...
function readCheckBox($ArrayCheckbox, $conn, $category_id){global $alert_message; //Ruft die Zuweisungsfunktion auf
  foreach ($ArrayCheckbox as $key => $user_id)
  {assignUserCategory($category_id, $user_id,$conn); $alert_message = "Änderungen wurden gespeichert";}
...
function delAssign($conn, $user_id,$category_id){ //Löscht eine Zuweisung
...
function delEveryAssign ($conn, $category_id){
  ...
  function existingAssign($user_id,$category_id,$conn){ //Überprüft ob die Kategorie dem Nutzer schon zugeordnet wurde.
  ...
  function ShowButton($conn){global $category_id;
    if(existUser($conn)){
...
function assignUserCategory($category_id,$user_id,$conn){ //Erstellt eine Zuweisung
  $stmt = $conn->prepare("INSERT INTO  user_has_categories (user_ID,category_category_id) VALUES (?,?)");
  $stmt->bind_param("ii", $user_id, $category_id); 
  $stmt->execute();
}
function alreadyAssigned($category_id,$user_id,$conn){ //Überprüft ob es schon zugewiesen worden ist.
...
function view_checkBox_assign_category_user($conn,$category_id){
  ...
  }}else{echo "<tr><td>Es existieren keine Nutzer zum hinzufügen</td></tr>";}  //Falls keinne Nutzer existieren
}
...
else{$alert_message = "Kategorie ist keinem Nutzer mehr zugeordnet"; delEveryAssign($conn,$category_id);}
}}
        ...
        <table class="table table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Vorname:</th>
                        <th>Nachname:</th>
                        <th><p class="text-right">Auswahl:</p></th>						
                    </tr>
                </thead>
