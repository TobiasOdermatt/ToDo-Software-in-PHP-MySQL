<?php
  ...
  if (!isset($_SESSION["user"]) || $_SESSION["role"] != "user"){ 
    ...
    function archiveToDo($ToDo_ID,$conn){  
      ...
      }else{mysqli_query($conn, "UPDATE to_do SET archieve = 'false' where ID = '$ToDo_ID'");
...
//Lädt den Editmode | Werte des ToDos werden geladen und der Seitentext ändert sich
    ...
    function permissionToAction($ToDo_ID,$conn){ 
        ...
        if(empty(mysqli_fetch_assoc($result)["ID"])){return false;}
        else{return true;}
      }
...
function generateCategorylist($conn,$user_ID,$edit_category_ID){ 
    ...
    while($row = $query->fetch_array()){
        ...
        }else{echo "<option value=".$categoryID.">".$categoryname."</option>";}
  ...
  if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(isset($_POST['writemode'])){//Dient zur unterscheidung von ToDo erstellen und Bearbeiten Modus
      ...
      if($_POST['writemode'] == "submit"){  //Beim Erstellen
          ...
          $alert_message = "Der ToDo wurde erfolgreich angelegt.";}else{
          ...
          else{$alert_class_name = "alert alert-danger";$alert_message = "Sie haben keine Berechtigung das ToDo zu, bearbeiten oder es existiert nicht.";}}
      }}
          
      function checkSelected($edit_priority, $currentValue){ //Lädt die Prioritätenanzeige für HTML
        if($edit_priority == $currentValue){return "selected";}else{return "";}}
    ...
    {if(isset($_GET['ID'])){         //Sollte ein ToDo bearbeitet werden, dann ist die Variabel ID gesetzt
      ...
      if(permissionToAction($editmode,$conn,$_SESSION["id"])){
        ...
        else{header("location: ../index.php?status=permissionfail");}
    ...
    elseif(isset($_GET['DEL'])){  //Sollte ein ToDo gelöscht werden, dann ist die Variabel DEL gesetzt.
      ...
      if(permissionToAction($delID,$conn,$_SESSION["id"])){ //Überprüft ob der Nutzer die Rechte dafür hat (Ob er ersteller dieser ToDo war.)
        delToDo($_GET['DEL'],$conn);}  //Funktion für die Löschung wird gestartet, die Variabel DEL wird mitgegeben.
        else{header("location: ../index.php?status=permissionfail");}}
    ...
    elseif(isset($_GET['archive'])){  //Sollte ein ToDo gelöscht werden, dann ist die Variabel DEL gesetzt.
          ...
          if(permissionToAction($archiveTo_Do_ID,$conn,$_SESSION["id"])){ //Überprüft ob der Nutzer die Rechte dafür hat (Ob er ersteller dieser ToDo war.)
            archiveToDo($archiveTo_Do_ID,$conn);}  //Funktion das Archivieren, wenn False dann wird es auf true gesetzt wenn true dann wird es auf false gesetzt.
            else{header("location: ../index.php?status=permissionfail");}}}
    

        if(!(empty($error))){$alert_class_name = "alert alert-danger"; $alert_message = $error;} //Fehler werden ausgeben

        if(!haveCategories($conn,$user_ID)){$alert_class_name = "alert alert-danger";
...
<html lang="de">
    <head>
        ...
        <nav class="navbar navbar-default">
             ...
             <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-home"></span> Home | ToDoVerwaltung</button></form>
             </div>  </div>
            ...
            <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-inbox"></span> ToDo erstellen</button></form>
    ...
    <div class="well well-sm"><h4><?php echo $site_title;?></h4>  </div>
    ...
    <form action="index.php" method="post">
        ...
        <div class="form-group">
          ...
          <input type="text" name="title" class="form-control" id="title"
                  ...
                  placeholder="Geben Sie einen Titel ein maximal 45 Zeichen."
                  maxlength="45"
                  required>
        </div>
        <!-- ToDo text -->
        ...
        <textarea class="form-control" id="text" name="text" rows="3"
                  placeholder="Geben Sie eine Beschreibung an maximal 300 Zeichen."
                  maxlength="300"
                  required><?php echo $edit_text ?></textarea>
        </div>
...
<!-- Priorität Dropdown menü checkSelected wird geladen falls ToDo bearbeitet wird -->
        ...
        <input type="range"  name="status" step="1" min="0" max="100" value="<?php echo $edit_status ?>" id="range" onchange="rangePrimary.value=value">
            <input type="hidden" class="form-label" id="rangePrimary" /></div> </div> <br>
